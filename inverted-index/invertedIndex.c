...
TfIdfList translate_list(FileList files, int D) {
    ...
    FileList temp = files;
    int cases = 0, appended_to_head = 0;
    for (; temp != NULL; cases++) temp = temp->next;
...
TfIdfList sort_list(TfIdfList combined_list) {
    ...
    for (int i = length - 2; i >= 0; i--) {
        ...
        for (int j = 0; j <= i; j++) {
            ...
            if (temp1->tfIdfSum < temp2->tfIdfSum) {
                swap(temp1, temp2);
            } else if (temp1->tfIdfSum == temp2->tfIdfSum && word_diff > 0) {
                swap(temp1, temp2);
            }
            temp1 = temp2;
            temp2 = temp2->next;
...
// Swap two nodes in a tfIdf list
void swap(TfIdfList temp1, TfIdfList temp2) {
    ...
    // Swap tfIdf
    double temp_val = temp1->tfIdfSum;
    temp1->tfIdfSum = temp2->tfIdfSum;
    temp2->tfIdfSum = temp_val;
