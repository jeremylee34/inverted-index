...
FileList insert_file(InvertedIndexBST node, char *file, int new);
...
InvertedIndexBST generateInvertedIndex(char *collectionFilename) {
    ...
    fclose(fp);
    // if (node == NULL) node = malloc(sizeof(struct InvertedIndexNode));
...
InvertedIndexBST insert_node(InvertedIndexBST node, char *word, char *file) {
    ...
    if (node == NULL) {
        ...
        if (strcmp(curr->filename, file) != 0) {    
            int empty = 0;
            node->fileList = insert_file(node, file, empty);
        }
    }
...
InvertedIndexBST new_node(char *word, char *file) {
    ...
    int new = 1;
    new_node->fileList = insert_file(new_node, file, new);

    return new_node;
}

// Update the fileList linked list
FileList insert_file(InvertedIndexBST node, char *file, int empty) {
...
void printInvertedIndex(InvertedIndexBST tree) {
    ...
    FILE *pf = fopen("invertedIndex.txt", "r+");
